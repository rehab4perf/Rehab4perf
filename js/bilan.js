...
var CR_MED_LEXIQUE = [
  /* ── Membre inférieur ── */
  ['SLS',                         'Force fonctionnelle du membre inférieur',                 'squats sur une jambe'],
  ['Hop Test',                    'Capacité de saut en longueur sur un pied',                ''],
  ['Réception — 80% Hop Test',    'Qualité de réception',                                    'saut en longueur sur une jambe'],
  ['Drop Jump — Temps contact',   'Temps de contact au sol',                                 'saut depuis une marche'],
  ['Drop Jump — RSI',             'Indice de raideur réactive',                              'saut depuis une marche'],
  ['Drop Jump H',                 'Détente verticale',                                       'saut depuis une marche'],
  ['Side Hop — Explosivité',      'Explosivité latérale',                                    'sauts latéraux sur une jambe'],
  ...
  ['SEBT — Postéro-latéral',      'Contrôle postural dynamique — direction postéro-latérale', 'allonge du pied en équilibre sur une jambe'],
  ['Single-Leg Stance Test',      'Équilibre en appui unipodal',                             ''],
  ['Figure-of-8 Hop Test',        'Agilité en changement de direction',                      'sauts en huit sur une jambe'],
  ['Overhead squat',              'Contrôle moteur global',                                  'squat bras levés'],
  ['Squat unipodal — qualité',    'Contrôle du membre inférieur',                            'squat sur une jambe'],
  ...
  ['Pliométrie verticale (qualitative)', 'Qualité de rebond',                                       'sauts verticaux enchaînés'],
  ['Pliométrie verticale',        'Qualité de rebond',                                       'sauts verticaux enchaînés'],
  /* ── Membre supérieur ── */
  ['PSET',                        'Endurance des muscles postérieurs de l\'épaule',          'maintien du bras écarté'],
  ['Shoulder Endurance',          'Endurance de l\'épaule',                                  ''],
  ['mCKCUEST',                    'Stabilité de l\'épaule en chaîne fermée',                 'touchers de main en appui facial'],
  ['Side Hold Rotation',          'Endurance du tronc et de la ceinture scapulaire',         'gainage latéral avec rotation'],
  ['ULRT',                        'Endurance rotatoire du membre supérieur',                 'rotations en appui sur les mains'],
  ['UQYBT',                       'Contrôle postural dynamique du membre supérieur',         'allonge de la main en appui facial'],
  /* ── Rachis ──
  ...
  ['Test de Sørensen',            'Endurance des extenseurs lombaires',                      'maintien du tronc à l\'horizontale'],
  ['Shirado',                     'Endurance des fléchisseurs du tronc',                     'maintien tête et épaules décollées'],
  ['PDSLRT',                      'Endurance des fléchisseurs de hanche et de la sangle abdominale', 'maintien des jambes tendues décollées']
];
...
function _crMedLabel(cle) {
  ...
  /* Decision du praticien : le nom du protocole entre parentheses alourdissait
     l'intitule sans rien apprendre au medecin — « Force fonctionnelle du membre
     inferieur (SLS) ». Seule la FONCTION reste en tete, et le GESTE la precise
     en petit juste dessous. La colonne « protocole » a ete retiree de la table
     avec son unique usage : un champ que plus personne ne lit finit par etre
     recopie de travers. */
  return t.e[1] + t.reste;
}
...
const TESTS = {
  ...
  'tb-ha-hanche': {type:'ortho', items:[
    ...
    'FABER <span style="font-size:.68rem;background:#EBF3FF;color:var(--accent);padding:1px 5px;border-radius:4px;font-weight:600;margin-left:4px">Différenciation</span> <span style="font-size:.68rem;color:var(--text3);font-weight:400;display:block">Flexion + ABD + RE, pied posé au-dessus de la patella controlatérale ; le kiné majore l’ABD en stabilisant le bassin.<br><b>Critère :</b> douleur / amplitude.<br><b>Hypothèse selon la localisation</b> — antérieure : coxarthrose, FAI, labrum · latérale : GTPS · postérieure : sacro-iliaque.<br>Se 41–98 %, Sp 18–100 %</span>',
    'AB-HEER Test <span style="font-size:.68rem;background:#EBF3FF;color:var(--accent);padding:1px 5px;border-radius:4px;font-weight:600;margin-left:4px">Instabilité</span> <span style="font-size:.68rem;color:var(--text3);font-weight:400;display:block">Décubitus dorsal, hanche en ABD 30° + RE ; le kiné pousse le grand trochanter vers l’avant.<br><b>Critère :</b> douleur / appréhension.<br><b>Hypothèse</b> — appréhension : instabilité · douleur : atteinte du labrum.<br>Se 80,6 %, Sp 89,4 % (une seule étude)</span>',
...
/* ── Asymétrie affichée ───────────────────────────────────────────────────
   Le LSI reste calculé partout comme avant — min/max en bilatéral, atteint
   sur sain en unilatéral. Seul l'AFFICHAGE bascule : on montre l'écart au
   lieu de la ressemblance. 80 % de symétrie devient 20 % d'asymétrie.

   Les seuils de couleur ne bougent pas : ils lisent toujours le LSI. C'est
   ce qui rend ce changement sûr — une seule chose change, le nombre affiché.

   En unilatéral le LSI peut dépasser 100 % : le côté atteint est alors
   MEILLEUR que le sain, et l'asymétrie devient négative. Le signe porte
...
/* ── Asymetrie INVERSEE — le cote atteint fait mieux que le cote sain ───────
   ...
   Le verdict reste VERT — decision du praticien : le critere EST atteint, et
   un ambre l'aurait fait lire comme un resultat a surveiller alors que le test
   est reussi. Ce qui distingue la ligne n'est donc pas sa couleur mais son
   SECOND niveau de lecture : « asymetrie inversee » s'ecrit sous « Validé »,
   plus petit. On garde le fait d'abord, la nuance ensuite.

   Convention portee par la chaine elle-meme : « <verdict> — <nuance> ». Les
   rendus de pastille la coupent sur ce tiret et posent la nuance en dessous.
   Aucun autre verdict n'en porte : la coupe ne mord que la ou elle doit.
...
function calcLunge() {
  ...
  if (ca < 0) {
    ...
    if (ca < 10) {
      ...
      cls='warn'; stat= 'Asymétrie > 10%';
      interp = 'Asymétrie = ' + asymTxt(lsi) + ' - asymetrie relative (Hoch 2011)';
    } else {
      cls='good'; stat='OK Symetrique et suffisant';
      interp = 'CA = ' + ca + ' cm (>=10 cm), diff = ' + diff.toFixed(1) + ' cm (<=1,5 cm)' + (!isNaN(lsi) ? ', Asym. = ' + asymTxt(lsi) : '');
    }
    lsiEl.className = 'measure-stat ' + cls;
  }
...
/* Ce test avait-il DEJA ete evalue avant ce bilan ?

   Deux indices, car aucun ne suffit seul :

   - une marque de reevaluation sur ce bloc dans un bilan anterieur ;
   - une vraie valeur dans l'etat fusionne des bilans anterieurs.

   « Vraie » exclut `false` : `_serializeBilan` parcourt TOUT le formulaire, donc
   chaque bilan enregistre chaque case, cochee ou non. Une case a `false` dans un
   bilan de juin ne dit pas que le test a ete fait ce jour-la — elle est
   indiscernable de « jamais evaluee ». C'est le raisonnement deja pose pour
   `_afTouched`, applique ici. Sans cette exclusion, aucun bloc a cases ne
   pourrait jamais etre reconnu comme une premiere evaluation. */
function _crDejaEvalue(fieldIds, ctx, prev) {
  for (var i = ctx.prevStart; i < _allBilans.length; i++) {
    var marques = _reevalLire(_allBilans[i].donnees || {});
    if (!marques || !marques.length) continue;
    for (var j = 0; j < fieldIds.length; j++) {
      var b = _reevalMap && _reevalMap[fieldIds[j]];
      if (b && marques.indexOf(b) !== -1) return true;
    }
  }
  for (var k = 0; k < fieldIds.length; k++) {
    var v = prev[fieldIds[k]];
    if (v === undefined || v === null) continue;
    if (v === false || String(v) === '' || String(v) === 'false') continue;
    return true;
  }
  return false;
}
...
function _crEtat(fieldIds) {
  ...
  if (reeval) {
    ...
    if (connu) {
      ...
      /* « Reevalue » sur un test fait pour la PREMIERE fois est faux, et le
         medecin le lira comme un suivi alors que c'est une mesure initiale. */
      if (!_crDejaEvalue(fieldIds, ctx, prev)) return 'initial';
...
function _crMarquage(fieldIds) {
  ...
  /* Une premiere mesure ne porte AUCUNE mention. La regle est celle du
     praticien, et elle se suffit : ce qui n'est pas marque « reevalue » est une
     premiere evaluation. Un libelle de plus dirait ce que l'absence dit deja,
     et alourdirait un document ou chaque ligne compte.

     L'etat reste distingue en interne : sans lui, une premiere mesure serait
     annoncee « reevaluee », ce qui est faux — c'est tout l'objet de ce cas. La
     barre d'accent a gauche marque, elle, que la ligne a bien ete mesuree dans
     ce bilan. */
  if (et === 'initial') return { cls: ' cr-item--reev', badge: '' };
...
function _crLegendeMarquage() {
  ...
  return '<div class="cr-legende">Bilan de suivi. Chaque résultat porte la date de sa '
       ...
       + 'italique proviennent d\'un bilan antérieur et n\'ont pas été recontrôlées. '
       + 'Une ligne sans mention est une première évaluation.</div>';
}
...
function _statForce(lsi) {
  ...
  if (lsiInverse(lsi)) return { txt: LSI_INVERSE_TXT,     cls: 'ok'   };
...
function _buildAllTestsHtml() {
  ...
  var lsiCls2 = function(ca,cs) {
    var v = lsiVal2(ca,cs);
    if (isNaN(v)) return '';
    /* Vert : au-dela de 110 le critere « >= 90 % » est largement franchi.
       C'est le libelle qui porte la nuance, pas la couleur. */
  ...
  for (var oi=0; oi<orthoSections.length; oi++) {
    ...
    if (sec.label === 'EPAULE') {
      ...
      if (!isNaN(girdCS) && !isNaN(girdCA)) {
        ...
        secRows += crItem('GIRD — RI',
          _crMesTab([{ l:'Rotation interne', a:girdCS+' °', b:girdCA+' °' }], _labelCS, _labelCA,
            { note: 'Différence : ' + (girdDiff>=0?'+':'') + girdDiff.toFixed(0) + ' °' }),
          girdTag, girdCls, ['gird-cs','gird-ca']);
      }
      // Force break tests
      var epForceTests = [
  ...
  /* ── Mesures d'une ligne de CR, en mini-tableau ────────────────────────────
     ...
     colonne. Remplace la phrase continue « G=11cm  D=9cm  Asym. = 18.2% » :
     elle melangeait trois separateurs (=, /, ·) et rejetait en fin de phrase
     le chiffre qu'on cherche. Les nombres sont en chasse fixe, donc alignes
     et comparables d'une ligne a l'autre.

     `rows` : [{ l:'Distance', a:'11 cm', b:'9 cm', asym:'18.2%' }]
     L'ordre des colonnes suit la convention de _p : Gauche d'abord en
     bilateral, cote atteint d'abord en unilateral. */
  var _mesTab = function(rows, opts) {
    return _crMesTab(rows, _isBilatMI ? _labelCS : _labelCA,
                           _isBilatMI ? _labelCA : _labelCS, opts);
  };

  /* Asymetrie seule, sans le prefixe « Asym. = » : la colonne le dit deja. */
  var _asymOf = function(ca, cs) {
    if (isNaN(ca) || isNaN(cs)) return '';
    var v = _isBilatMI ? (ca > 0 && cs > 0 ? Math.min(ca,cs)/Math.max(ca,cs)*100 : NaN)
                       : (cs > 0 ? ca/cs*100 : NaN);
    return isNaN(v) ? '' : asymTxt(v);
  };

  /* Cas courant : une mesure, deux cotes, une asymetrie. */
  var _pair = function(label, caVal, csVal, unit, asym) {
    var u = unit || '';
    var f = function(v){
    ...
    return _mesTab([{ l: label, a: f(vA), b: f(vB), asym: asym || '' }]);
  };
  ...
  if (!isNaN(hopCA)) {
    var hopNote = [];
    if (!isNaN(hopCS) && hopCS > 0) hopNote.push('Repère 80 % CS = ' + (hopCS * 0.8).toFixed(1) + ' cm');
    if (!isNaN(hopTaille) && hopTaille > 0) {
      if (hopSexe === 'F') { var hopSeuil = (hopTaille * 0.80).toFixed(1); hopNote.push('Seuil RTS 80 % taille (F) = ' + hopSeuil + ' cm → ' + (hopCA >= parseFloat(hopSeuil) ? '✓ atteint' : '✗ non atteint')); }
      else if (hopSexe === 'H') { var hopSeuil = (hopTaille * 0.90).toFixed(1); hopNote.push('Seuil RTS 90 % taille (H) = ' + hopSeuil + ' cm → ' + (hopCA >= parseFloat(hopSeuil) ? '✓ atteint' : '✗ non atteint')); }
      else { hopNote.push('Seuil RTS : ≥ ' + (hopTaille * 0.80).toFixed(1) + ' cm (F) / ≥ ' + (hopTaille * 0.90).toFixed(1) + ' cm (H)'); }
    }
  ...
  if (!isNaN(luCA)) {
    ...
    tfHtml += crItem('Lunge WBLT',
      _mesTab([{ l:'Distance', a:(_isBilatMI?luCS:luCA)+' cm', b:(_isBilatMI?luCA:luCS)+' cm', asym:_asymOf(luCA,luCS) }],
        { note: 'Différence : ' + luDiff2.toFixed(1) + ' cm' }),
  ...
  if (slstTouched) {
    ...
    tfHtml += crItem('Single-Leg Stance Test', _pair('Erreurs', slstCATotal+' err.', slstCSTotal+' err.', ''), slstStat, slstCls, ['slst-ca-1','slst-cs-1']);
  }
  ...
  if (!isNaN(f8CA)) {
    ...
    var f8Lsi_s = isNaN(f8Lsi) ? '' : '   Asym. = ' + asymTxt(f8Lsi);
    var f8Seuil = f8CA <= 12 ? '   Seuil <12s ✓' : '   >12s ✗';
    tfHtml += crItem('Figure-of-8 Hop Test',
      _mesTab([{ l:'Temps', a:(_isBilatMI?(!isNaN(f8CS)?f8CS+' s':''):f8CA+' s'), b:(_isBilatMI?f8CA+' s':(!isNaN(f8CS)?f8CS+' s':'')), asym:(isNaN(f8Lsi)?'':asymTxt(f8Lsi)) }],
        { note: f8CA <= 12 ? 'Seuil < 12 s ✓' : 'Seuil < 12 s ✗' }),
      f8Cls==='good' ? 'Asym. ≤10%' : f8Cls==='warn' ? 'Asym. 11-20%' : f8Cls==='bad' ? 'Asym. >20%' : '',
      f8Cls, ['f8-ca','f8-cs']);
  }
  ...
  var lsiTagFn2 = function(v){ return v>=90?'Asym. ≤ 10 % — OK':v>=80?'Asym. 10–20 % — Acceptable':'Asym. > 20 % — Insuffisant'; };
      ...
      if (!isNaN(csN) && csN > 0) {
        ...
        if (ft.key === 'ep-dent') {
          ...
          if (!isNaN(caN)) {
            ...
            if (!isNaN(lsiDent) && lsiDent < 90) caReasons.push('Asym. ' + asymTxt(lsiDent, 0));
          ...
          if (!isNaN(caN) && csN > 0 && caN/csN*100 < 90) { var i4 = ft.label + ' — ' + _labelCA + ' (Asym. ' + asymTxt((caN/csN*100), 0) + ')'; if (toWork.indexOf(i4)<0) toWork.push(i4); }
        }
      } else {
        // Mode appréciation
  ...
  if (!isNaN(slsLsi2) && slsLsi2<90) toWork.push('Renforcement MI — squat unilatéral (Asym. SLS ' + asymTxt(slsLsi2) + ')');
  if (!isNaN(hopLsi2) && hopLsi2<90) toWork.push('Travail de propulsion — saut unipodal (Asym. Hop ' + asymTxt(hopLsi2) + ')');
  ...
  if (!isNaN(sebtPmCA2) &&!isNaN(sebtPmCS2) &&sebtPmCS2>0 &&sebtPmCA2/sebtPmCS2*100<90) { sebtDeficit=true; toWork.push('Améliorer contrôle postural dynamique — SEBT postéro-médial (Asym. ' + asymTxt((sebtPmCA2/sebtPmCS2*100), 1) + ')'); }
  if (!isNaN(sebtPlCA2) &&!isNaN(sebtPlCS2) &&sebtPlCS2>0 &&sebtPlCA2/sebtPlCS2*100<90) { sebtDeficit=true; toWork.push('Améliorer contrôle postural dynamique — SEBT postéro-latéral (Asym. ' + asymTxt((sebtPlCA2/sebtPlCS2*100), 1) + ')'); }
  // UQYBT
...
function calcULRT() {
  ...
  if (!isNaN(moyD) && !isNaN(moyG) && Math.max(moyD,moyG) > 0) {
    ...
    if (elLsi) {
      elLsi.textContent = 'Asymétrie = ' + asymTxt(lsi) + '  (seuil significatif > 10%)';
  ...
  if (!isNaN(cs) && cs > 0) {
    ...
    if (key === 'ep-dent') {
      ...
      if (dentPos) {
        ...
        if (!isNaN(lsi) && lsi < 90) reasons.push('Asym. ' + asymTxt(lsi, 0));
...
(function(){
  ...
  function _ctLsiText(lsi){
    return isNaN(lsi) ? '—' : asymTxt(lsi, 0);
  }
