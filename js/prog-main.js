...
function renderCalendar() {
  if(_progPatient) {
    ...
    var url = SUPA_URL_P + '/rest/v1/seances_planifiees?patient_id=eq.' + _fetchPatientId
            + '&select=id,date,programme_id,programmes(nom,donnees),athlete_feedback(rpe,duree_min,douleur,effort,exo_data)&order=date.asc';
    ...
    _fetchRetry(url, { headers: calHeaders })
      ...
      .then(function(data){
        ...
        var stravaUrl = SUPA_URL_P + '/rest/v1/strava_activities?patient_id=eq.' + _fetchPatientId
          + '&select=strava_id,date,type,nom,distance_m,duree_s,charge,donnees,seance_id&order=date.asc';
...
/* _uaFoster, _stravaChargeEstimate et l'agrégation _volumeParSport vivent dans
   js/volume-sport.js : l'espace athlète calcule son volume avec les mêmes. */
...
/* ── Helpers sémantiques feedback ──────────────────────────────────
   ...
   Fallback legacy : anciennes lignes CAP ou rpe=douleur et duree_min=effort<=10. */
function _fbDouleur(fb){
  if(!fb) return null;
  if(fb.douleur !== null && fb.douleur !== undefined) return fb.douleur;
...
function _fbEffort(fb){
  ...
  if(fb.duree_min && fb.duree_min <= 10) return fb.duree_min; // legacy CAP
  return null;
}
/* _fbIsCharge et _evIsCap vivent dans js/volume-sport.js. */
...
function _buildUaMap(){
  ...
  _stravaActivities.forEach(function(act){
    if(act.seance_id || !act.date) return;
    var charge = _stravaChargeEstimate(act, ddn);
    if(charge) map[act.date] = (map[act.date]||0) + charge;
  });
  return map;
}
...
function _crChargeHtml(uaMap, vol, aujIso){
  ...
  var style = '<style>'
    ...
    + '.crc-t{width:auto;border-collapse:collapse;margin:0 0 8px}'
    ...
    + '.crc-t .n{text-align:right;font-variant-numeric:tabular-nums;white-space:nowrap;width:86px}'
...
function _buildDayChips(dateStr, cellDate, _skipCap){
  var allChips = [];
  // Deduplication par lien persistant : une activite Strava liee (seance_id)
  // est absorbee en badge dans la chip de sa seance, quel que soit le type.
  ...
  if(_progPatient){
    ...
    allChips = dayEvs.map(function(ev){
      ...
      if (nom.indexOf('CAP —') === 0) {
        ...
        var painScore = _fbDouleur(fbCap);
        var painBadge = painScore !== null
          ...
          : '';
        var capStravaBadge = _stravaBadge(_linkedAct[String(ev.id)]);
        ...
        if (_calSelMode) {
          ...
          return '<div id="cal-chip-'+ev.id+'" class="cal-session-chip" style="background:'+capBg+';color:#fff;cursor:pointer'+capSelRing+'" title="'+escH(nom)+'" onclick="event.stopPropagation();_calSelToggle(\''+ev.id+'\',\'seance\')">'
            + '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;">🏃 '+escH(capLabel.length>15?capLabel.slice(0,15)+'…':capLabel)+painBadge+capStravaBadge+'</span>'
  ...
  // Chips Strava — activites NON liees uniquement (les liees sont absorbees en badge)
  if(_progPatient && _stravaActivities.length) {
    var _unlinked = _stravaDay.filter(function(a){ return !a.seance_id; });
    var _typeMap = { Run:'🏃', Ride:'🚴', Swim:'🏊', Walk:'🚶', WeightTraining:'💪', Hike:'🥾', Rowing:'🚣', VirtualRide:'🚴', TrailRun:'🏔️' };
    if(_unlinked.length >= 2){
      ...
      _unlinked.forEach(function(act){
        var emoji = _typeMap[act.type] || '⚡';
...
function _feedbackRenderContent(fb, sid) {
  ...
  if (!hasAthleteData) {
    ...
    // CAP : colonnes dediees douleur (0-10) et effort Borg (1-10)
    var pain = _fbDouleur(fb);
    var effort = _fbEffort(fb);
...
function _fbMaxPain(fb){
  ...
  // Douleur seance (CAP/HSR) via colonne dediee (+ fallback legacy)
  if(mx === null) mx = _fbDouleur(fb);
  return mx;
}
...
function _renderAgendaProgList(type) {
  ...
  var rows = Object.values(groups).map(function(g){
    ...
    return '<div style="display:flex;align-items:center;gap:8px;margin-top:6px;padding:8px 10px;'
      ...
      + 'font-size:.7rem;padding:3px 9px;cursor:pointer;font-family:inherit;white-space:nowrap;"><svg style="vertical-align:middle;margin-right:4px" width="16" height="16" fill="currentColor" viewBox="-40 0 427 427.00131" xmlns="http://www.w3.org/2000/svg"><path d="m232.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m114.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m28.398438 127.121094v246.378906c0 14.5625 5.339843 28.238281 14.667968 38.050781 9.285156 9.839844 22.207032 15.425781 35.730469 15.449219h189.203125c13.527344-.023438 26.449219-5.609375 35.730469-15.449219 9.328125-9.8125 14.667969-23.488281 14.667969-38.050781v-246.378906c18.542968-4.921875 30.558593-22.835938 28.078124-41.863282-2.484374-19.023437-18.691406-33.253906-37.878906-33.257812h-51.199218v-12.5c.058593-10.511719-4.097657-20.605469-11.539063-28.03125-7.441406-7.421875-17.550781-11.5546875-28.0625-11.46875h-88.796875c-10.511719-.0859375-20.621094 4.046875-28.0625 11.46875-7.441406 7.425781-11.597656 17.519531-11.539062 28.03125v12.5h-51.199219c-19.1875.003906-35.394531 14.234375-37.878907 33.257812-2.480468 19.027344 9.535157 36.941407 28.078126 41.863282zm239.601562 279.878906h-189.203125c-17.097656 0-30.398437-14.6875-30.398437-33.5v-245.5h250v245.5c0 18.8125-13.300782 33.5-30.398438 33.5zm-158.601562-367.5c-.066407-5.207031 1.980468-10.21875 5.675781-13.894531 3.691406-3.675781 8.714843-5.695313 13.925781-5.605469h88.796875c5.210937-.089844 10.234375 1.929688 13.925781 5.605469 3.695313 3.671875 5.742188 8.6875 5.675782 13.894531v12.5h-128zm-71.199219 32.5h270.398437c9.941406 0 18 8.058594 18 18s-8.058594 18-18 18h-270.398437c-9.941407 0-18-8.058594-18-18s8.058593-18 18-18zm0 0"/><path d="m173.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/></svg>Supprimer</button>'
      + '</div>';
  });
...
function touchSheetLink(){
  if(!_touchSheetData) return;
  var evId   = _touchSheetData.evId;
  var progId = _touchSheetData.progId;
  closeTouchSheet();
  _shareSeanceLink(evId, progId);
}

function _shareSeanceLink(evId, progId){
  var link = _athleteLink(progId) + '&seance=' + evId;
  if(navigator.clipboard && navigator.clipboard.writeText){
    navigator.clipboard.writeText(link)
      .then(function(){ _showToast('🔗 Lien séance copié !'); })
...
function _openTmplModal(opts){
  ...
  document.getElementById('tmplModalTitle').textContent = _tmplEditId ? 'Modifier le modèle' : 'Enregistrer un modèle';
  document.getElementById('tmplModalConfirmBtn').innerHTML = _tmplEditId ? _PROG_SAVE_ICON+'Enregistrer les modifications' : _PROG_SAVE_ICON+'Enregistrer le modèle';
...
function openEditTemplate(id){
  ...
  .then(function(r){ return r.json(); })
  .then(function(data){
    if(!Array.isArray(data)||!data[0]){ alert('Modèle introuvable.'); return; }
    var t = data[0];
    _openTmplModal({ editId:id, nom:t.nom, cat:t.categorie, type:t.type, emoji:t.emoji, groupId:t.group_id, phaseNom:t.phase_nom, phaseOrdre:t.phase_ordre, pathologie:t.pathologie, sport:t.sport });
  })
...
function doSaveTemplate(){
  ...
  if(!nom){ alert('Donnez un nom au modèle.'); return; }
...
function _renderSidebarPicker(){
  ...
  if(!html) html = '<div class="picker-empty">'+(search?'Aucun résultat pour « '+escH(_sbPickerSearch)+' »':'Aucun modèle disponible.')+'</div>';
...
function unpublishLibTemplate(id){
  _confirmDialog({id:'cd-unpublish-lib', emoji:'📚', title:'Retirer ce modèle ?', body:'Ce modèle sera retiré de la bibliothèque partagée.', confirmLabel:'Retirer', confirmColor:'#d97706'}, function(){
    ...
    .then(function(r){
      if(r.ok||r.status===204){
        _showToast('Modèle retiré de la bibliothèque partagée');
...
function _refreshSaveBtn(){
  ...
  if(_builderMode === 'template'){
    btn.innerHTML = _PROG_SAVE_ICON + 'Enregistrer le modèle';
    btn.title = 'Enregistrer ce contenu comme modèle';
...
function _doUpdateTemplate(){
  ...
  .then(function(res){
    ...
    if(!res.ok){ _refreshSaveBtn(); _handleApiError(res.status, res.data, 'mise à jour modèle'); return; }
    _showToast('Modèle mis à jour');
    _draftClear();
    _refreshSaveBtn();
    renderSidebarTemplates();
  })
...
function duplicateTemplate(){
  if(!_builderFromTemplate){ _showToast('Aucun modèle chargé.'); return; }
  if(!_progToken || !_progUid){ alert('Session non disponible.'); return; }
  var src = (_sidebarProgs||[]).find(function(t){ return String(t.id)===String(_builderFromTemplate); });
  var newNom = 'Copie de ' + (src ? (src.nom||src.phase_nom||'Modèle') : 'Template');
  var donnees = JSON.stringify({ blocs: JSON.parse(JSON.stringify(blocs||[])), etapes: JSON.parse(JSON.stringify(etapes||[])), notes: getNotes() });
  var payload = {
  ...
  .then(function(data){
    ...
    renderSidebarTemplates();
    _showToast('Modèle « '+newNom+' » dupliqué !');
  })
  ...
  function _applyTemplate(t){
    ...
    if(ouvrirModele){
      ...
      /* Le champ de nom EST ce qui sera re-enregistre : le laisser vide, c'est
         effacer le nom du repertoire a la premiere mise a jour. On l'affiche
         donc, et l'utilisateur voit ce sur quoi il travaille. */
      var _pnEl = document.getElementById('patientName');
...
function deleteTemplate(id){
  _confirmDialog({id:'cd-del-template', emoji:'🗑️', title:'Supprimer ce modèle ?', body:'Cette action est irréversible.', confirmLabel:'Supprimer'}, function(){
    if(_progToken && _progUid){
      _fetchRetry(SUPA_URL_P + '/rest/v1/templates?id=eq.' + id, {
        method: 'DELETE', headers: _sbHeaders()
      }).then(function(r){
...
function _doPublishGroup(groupId, nom, cat, desc){
  ...
  .then(function(r){
    if(r.ok||r.status===201){
      _showToast('🌐 Protocole publié dans la bibliothèque partagée !');
    } else {
...
function _doUpdateGroupInLib(libId, groupId, nom, cat, desc){
  ...
  .then(function(r){
    if(r.ok) _showToast('🌐 Bibliothèque mise à jour !');
      ...
      alert('Erreur mise à jour bibliothèque partagée ('+r.status+') : '+t);
    });
  })
...
function _doUnpublishGroup(libId){
  ...
  .then(function(r){
    if(r.ok||r.status===204) _showToast('🔒 Protocole retiré de la bibliothèque partagée.');
    else r.text().then(function(t){
...
function _openKebab(e, type, id, nom, ownerId){
  ...
  if(isOwner){
    items.push(type === 'group'
      ? { label: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg" style="vertical-align:middle;margin-right:5px"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>  Modifier', action: function(){ openEditGroup(id); } }
      : { label: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg" style="vertical-align:middle;margin-right:5px"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>  Modifier', action: function(){ openEditTemplate(id); } });
  }
  items.push(type === 'group'
  ...
  if(isOwner){
    items.push(type === 'group'
      ? { label: '<svg width="14" height="14" viewBox="-40 0 427 427.00131" fill="currentColor" xmlns="http://www.w3.org/2000/svg" style="vertical-align:middle;margin-right:5px"><path d="m232.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m114.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m28.398438 127.121094v246.378906c0 14.5625 5.339843 28.238281 14.667968 38.050781 9.285156 9.839844 22.207032 15.425781 35.730469 15.449219h189.203125c13.527344-.023438 26.449219-5.609375 35.730469-15.449219 9.328125-9.8125 14.667969-23.488281 14.667969-38.050781v-246.378906c18.542968-4.921875 30.558593-22.835938 28.078124-41.863282-2.484374-19.023437-18.691406-33.253906-37.878906-33.257812h-51.199218v-12.5c.058593-10.511719-4.097657-20.605469-11.539063-28.03125-7.441406-7.421875-17.550781-11.5546875-28.0625-11.46875h-88.796875c-10.511719-.0859375-20.621094 4.046875-28.0625 11.46875-7.441406 7.425781-11.597656 17.519531-11.539062 28.03125v12.5h-51.199219c-19.1875.003906-35.394531 14.234375-37.878907 33.257812-2.480468 19.027344 9.535157 36.941407 28.078126 41.863282zm239.601562 279.878906h-189.203125c-17.097656 0-30.398437-14.6875-30.398437-33.5v-245.5h250v245.5c0 18.8125-13.300782 33.5-30.398438 33.5zm-158.601562-367.5c-.066407-5.207031 1.980468-10.21875 5.675781-13.894531 3.691406-3.675781 8.714843-5.695313 13.925781-5.605469h88.796875c5.210937-.089844 10.234375 1.929688 13.925781 5.605469 3.695313 3.671875 5.742188 8.6875 5.675782 13.894531v12.5h-128zm-71.199219 32.5h270.398437c9.941406 0 18 8.058594 18 18s-8.058594 18-18 18h-270.398437c-9.941407 0-18-8.058594-18-18s8.058593-18 18-18zm0 0"/><path d="m173.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/></svg>  Supprimer', action: function(){ deleteGroup(id); }, danger: true }
      : { label: '<svg width="14" height="14" viewBox="-40 0 427 427.00131" fill="currentColor" xmlns="http://www.w3.org/2000/svg" style="vertical-align:middle;margin-right:5px"><path d="m232.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m114.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m28.398438 127.121094v246.378906c0 14.5625 5.339843 28.238281 14.667968 38.050781 9.285156 9.839844 22.207032 15.425781 35.730469 15.449219h189.203125c13.527344-.023438 26.449219-5.609375 35.730469-15.449219 9.328125-9.8125 14.667969-23.488281 14.667969-38.050781v-246.378906c18.542968-4.921875 30.558593-22.835938 28.078124-41.863282-2.484374-19.023437-18.691406-33.253906-37.878906-33.257812h-51.199218v-12.5c.058593-10.511719-4.097657-20.605469-11.539063-28.03125-7.441406-7.421875-17.550781-11.5546875-28.0625-11.46875h-88.796875c-10.511719-.0859375-20.621094 4.046875-28.0625 11.46875-7.441406 7.425781-11.597656 17.519531-11.539062 28.03125v12.5h-51.199219c-19.1875.003906-35.394531 14.234375-37.878907 33.257812-2.480468 19.027344 9.535157 36.941407 28.078126 41.863282zm239.601562 279.878906h-189.203125c-17.097656 0-30.398437-14.6875-30.398437-33.5v-245.5h250v245.5c0 18.8125-13.300782 33.5-30.398438 33.5zm-158.601562-367.5c-.066407-5.207031 1.980468-10.21875 5.675781-13.894531 3.691406-3.675781 8.714843-5.695313 13.925781-5.605469h88.796875c5.210937-.089844 10.234375 1.929688 13.925781 5.605469 3.695313 3.671875 5.742188 8.6875 5.675782 13.894531v12.5h-128zm-71.199219 32.5h270.398437c9.941406 0 18 8.058594 18 18s-8.058594 18-18 18h-270.398437c-9.941407 0-18-8.058594-18-18s8.058593-18 18-18zm0 0"/><path d="m173.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/></svg>  Supprimer', action: function(){ deleteTemplate(id); }, danger: true });
  }
...
function _stravaSetLink(stravaId, seanceId){
  var drop = document.getElementById('_stravaDetailDrop');
  if(drop) drop.remove();
  _fetchRetry(SUPA_URL_P + '/rest/v1/strava_activities?strava_id=eq.' + stravaId, {
    method: 'PATCH',
    headers: Object.assign({}, _sbHeaders(), { 'Prefer': 'return=minimal' }),
    body: JSON.stringify({ seance_id: seanceId })
  }).then(function(r){
    if(!r.ok){ alert('Impossible de modifier le lien (droits ou réseau).'); return; }
    var act = _stravaActivities.find(function(a){ return a.strava_id === stravaId; });
...
/* ── Popover groupe d'activites Strava non liees d'un jour ── */
function _showStravaGroup(e, dateStr){
  e.stopPropagation();
  var existing = document.getElementById('_stravaDetailDrop');
  if(existing) existing.remove();
  var acts = _stravaActivities.filter(function(a){ return a.date === dateStr && !a.seance_id; });
  if(!acts.length) return;
  var typeMap = { Run:'🏃', Ride:'🚴', Swim:'🏊', Walk:'🚶', WeightTraining:'💪', Hike:'🥾', Rowing:'🚣', VirtualRide:'🚴', TrailRun:'🏔️' };
  var drop = document.createElement('div');
  drop.className = 'stmpl-kdrop';
  drop.id = '_stravaDetailDrop';
  drop.style.cssText = 'min-width:210px;max-width:270px;padding:10px 12px;';
  var html = '<div style="font-weight:700;font-size:.78rem;color:#FC4C02;margin-bottom:6px;">Activités Strava — '+escH(dateStr.split('-').reverse().join('/'))+'</div>';
  acts.forEach(function(a){
    var dist = a.distance_m ? (a.distance_m >= 1000 ? (a.distance_m/1000).toFixed(1)+'km' : a.distance_m+'m') : '';
    var dur  = a.duree_s ? Math.round(a.duree_s/60)+'min' : '';
    var meta = [dist, dur].filter(Boolean).join(' · ');
    html += '<button style="width:100%;display:flex;justify-content:space-between;gap:8px;padding:6px 4px;border:none;border-bottom:1px solid var(--border);background:none;font-size:.76rem;cursor:pointer;text-align:left;" '
      + 'onclick="event.stopPropagation();_showStravaDetail(event,'+a.strava_id+')">'
      + '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+(typeMap[a.type]||'⚡')+' '+escH((a.nom||a.type||'Activité').slice(0,22))+'</span>'
      + '<span style="color:var(--text3);flex-shrink:0;">'+escH(meta)+'</span>'
      + '</button>';
  });
  drop.innerHTML = html;
  document.body.appendChild(drop);
  var rect = e.currentTarget.getBoundingClientRect();
  var dropW = drop.offsetWidth || 230, dropH = drop.offsetHeight || 150;
  var left = rect.left, top = rect.bottom + 6;
  if(left + dropW > window.innerWidth - 8) left = window.innerWidth - dropW - 8;
  if(top + dropH > window.innerHeight - 8) top = rect.top - dropH - 6;
  drop.style.left = left + 'px'; drop.style.top = top + 'px';
  setTimeout(function(){
    document.addEventListener('click', function rm(){ var d=document.getElementById('_stravaDetailDrop'); if(d) d.remove(); document.removeEventListener('click', rm, true); }, { once: true, capture: true });
  }, 0);
}
...
function _switchToCalendarPlanMode(){
  ...
  /* `_planOverrideProgId` marque une planification lancee depuis une carte de
     repertoire : on ne vient pas davantage du builder que depuis le menu d'une
     seance. Sans cette condition, `_builderDate` — survivant d'une seance
     ouverte plus tot — pre-cochait un jour au hasard, et l'on croyait avoir
     valide une date par megarde en touchant l'option. */
  var depuisBuilder = !_touchSheetData && !_planOverrideProgId;
  if(_touchSheetData){
...
function duplicateTemplateById(id){
  ...
  if(!src){ alert('Modèle introuvable.'); return; }
  var newNom = 'Copie de ' + (src.phase_nom||src.nom||'Modèle');
...
function _renderSidebarWithGroups(){
  ...
  var html = '<div class="stmpl-group-add-row">'
    + '<button class="stmpl-new-tmpl-btn" onclick="nouveauTemplate()">' + _TMPL_ICON + 'Nouveau modèle</button>'
  ...
  if(!hasContent){
    html += '<div class="stmpl-empty">Aucun modèle'+(catFilter?' dans cette catégorie':'')+'.<br>Créez-en un depuis le builder (📋 Enregistrer).</div>';
  }
  scroll.innerHTML = html;
}
...
function _renderTmplCard(p, isPhase, phaseNum, hideCat, groupe){
  ...
  var pubBdg = p.is_public ? '<span class="stmpl-public-badge" title="Modèle public">'+_ICO_PUBLIC+'</span>' : '';
...
function nouveauTemplate(opts){
  ...
  _showToast(((opts&&opts.groupNom) ? 'Nouvelle phase de « '+opts.groupNom+' »' : 'Nouveau modèle')
             + ' — composez, puis « Enregistrer le modèle »');
}
...
/* Le bouton topbar ouvre le même panneau latéral dans les deux modes
   (Exercices en builder, Bibliotheque sur l'agenda) :
...
/* PROTOCOLS_REF — la bibliothèque intégrée — vit dans js/protocoles-ref.js,
   chargé avant ce fichier par programme.html et par athlete.html. */
...
function renderProtocols() {
  ...
  var fetchSeances = _fetchRetry(SUPA_URL_P + '/rest/v1/seances_planifiees?patient_id=eq.' + patientId
    + '&select=id,programmes(donnees),athlete_feedback(rpe,douleur)',
    { headers: _sbHeaders() }).then(function(r){ return r.ok ? r.json() : []; });
  ...
  .then(function(results){
    ...
    seances.forEach(function(ev){
      ...
      if(ev.athlete_feedback && (ev.athlete_feedback.rpe || _fbDouleur(ev.athlete_feedback) !== null)) seancesStats[lp.protoId][lp.phaseId].done++;
    });
...
function _renderProtocolCard(proto) {
  ...
  return '<div class="proto-card" id="proto-card-'+proto.id+'">'
    ...
    + ((_isAdmin()) ? '<div class="proto-admin-btns r4p-admin-only" onclick="event.stopPropagation()">'
      + '<button class="proto-admin-btn proto-admin-btn--edit" onclick="openProtoEditor(\''+proto.id+'\')"><svg width="14" height="14" fill="currentColor" viewBox="0 0 348.882 348.882" xmlns="http://www.w3.org/2000/svg"><g><path d="M333.988,11.758l-0.42-0.383C325.538,4.04,315.129,0,304.258,0c-12.187,0-23.888,5.159-32.104,14.153L116.803,184.231c-1.416,1.55-2.49,3.379-3.154,5.37l-18.267,54.762c-2.112,6.331-1.052,13.333,2.835,18.729c3.918,5.438,10.23,8.685,16.886,8.685c0,0,0.001,0,0.001,0c2.879,0,5.693-0.592,8.362-1.76l52.89-23.138c1.923-0.841,3.648-2.076,5.063-3.626L336.771,73.176C352.937,55.479,351.69,27.929,333.988,11.758z M130.381,234.247l10.719-32.134l0.904-0.99l20.316,18.556l-0.904,0.99L130.381,234.247z M314.621,52.943L182.553,197.53l-20.316-18.556L294.305,34.386c2.583-2.828,6.118-4.386,9.954-4.386c3.365,0,6.588,1.252,9.082,3.53l0.419,0.383C319.244,38.922,319.63,47.459,314.621,52.943z"/><path d="M303.85,138.388c-8.284,0-15,6.716-15,15v127.347c0,21.034-17.113,38.147-38.147,38.147H68.904c-21.035,0-38.147-17.113-38.147-38.147V100.413c0-21.034,17.113-38.147,38.147-38.147h131.587c8.284,0,15-6.716,15-15s-6.716-15-15-15H68.904c-37.577,0-68.147,30.571-68.147,68.147v180.321c0,37.576,30.571,68.147,68.147,68.147h181.798c37.576,0,68.147-30.571,68.147-68.147V153.388C318.85,145.104,312.134,138.388,303.85,138.388z"/></g></svg></button>'
      + '<button class="proto-admin-btn proto-admin-btn--del" onclick="_deleteCustomProto(\''+proto.id+'\')"><svg width="14" height="14" fill="currentColor" viewBox="-40 0 427 427.00131" xmlns="http://www.w3.org/2000/svg"><path d="m232.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m114.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m28.398438 127.121094v246.378906c0 14.5625 5.339843 28.238281 14.667968 38.050781 9.285156 9.839844 22.207032 15.425781 35.730469 15.449219h189.203125c13.527344-.023438 26.449219-5.609375 35.730469-15.449219 9.328125-9.8125 14.667969-23.488281 14.667969-38.050781v-246.378906c18.542968-4.921875 30.558593-22.835938 28.078124-41.863282-2.484374-19.023437-18.691406-33.253906-37.878906-33.257812h-51.199218v-12.5c.058593-10.511719-4.097657-20.605469-11.539063-28.03125-7.441406-7.421875-17.550781-11.5546875-28.0625-11.46875h-88.796875c-10.511719-.0859375-20.621094 4.046875-28.0625 11.46875-7.441406 7.425781-11.597656 17.519531-11.539062 28.03125v12.5h-51.199219c-19.1875.003906-35.394531 14.234375-37.878907 33.257812-2.480468 19.027344 9.535157 36.941407 28.078126 41.863282zm239.601562 279.878906h-189.203125c-17.097656 0-30.398437-14.6875-30.398437-33.5v-245.5h250v245.5c0 18.8125-13.300782 33.5-30.398438 33.5zm-158.601562-367.5c-.066407-5.207031 1.980468-10.21875 5.675781-13.894531 3.691406-3.675781 8.714843-5.695313 13.925781-5.605469h88.796875c5.210937-.089844 10.234375 1.929688 13.925781 5.605469 3.695313 3.671875 5.742188 8.6875 5.675782 13.894531v12.5h-128zm-71.199219 32.5h270.398437c9.941406 0 18 8.058594 18 18s-8.058594 18-18 18h-270.398437c-9.941407 0-18-8.058594-18-18s8.058593-18 18-18zm0 0"/><path d="m173.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/></svg></button>'
      + '</div>' : '')
  ...
  var criteriaItems = ph.exitCriteria.map(function(c, i){
    ...
    if(isChecked){
      ...
      dateStr = '<span class="proto-ci-date-wrap">'
        ...
        + '<button class="proto-ci-cal-btn" onclick="event.stopPropagation();_protoEditCheckedAt(\''+escJS(String(protoId))+'\',\''+escJS(ph.id)+'\','+i+')" title="Modifier la date"><svg width="14" height="14" fill="currentColor" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M452,311.145V75c0-24.813-20.187-45-45-45h-15V15c0-8.284-6.716-15-15-15s-15,6.716-15,15v15h-60V15c0-8.284-6.716-15-15-15s-15,6.716-15,15v15h-92V15c0-8.284-6.716-15-15-15s-15,6.716-15,15v15H90V15c0-8.284-6.716-15-15-15S60,6.716,60,15v15H45C20.187,30,0,50.187,0,75v331c0,24.813,20.187,45,45,45h266.946c16.795,36.01,53.085,61,95.054,61c57.897,0,105-47.551,105-106C512,364.198,487.443,328.027,452,311.145z M30,75c0-8.271,6.729-15,15-15h15v15c0,8.284,6.716,15,15,15s15-6.716,15-15V60h60v15c0,8.284,6.716,15,15,15s15-6.716,15-15V60h92v15c0,8.284,6.716,15,15,15s15-6.716,15-15V60h60v15c0,8.284,6.716,15,15,15s15-6.716,15-15V60h15c8.271,0,15,6.729,15,15v46H30V75z M303.056,421H45c-8.271,0-15-6.729-15-15V151h392v151.076c-4.901-0.704-9.907-1.076-15-1.076c-57.897,0-105,47.103-105,105C302,411.092,302.365,416.098,303.056,421z M407,482c-41.355,0-75-34.093-75-76c0-41.355,33.645-75,75-75c41.355,0,75,33.645,75,75C482,447.907,448.355,482,407,482z"/><path d="M437,391h-15v-15c0-8.284-6.716-15-15-15s-15,6.716-15,15v30c0,8.284,6.716,15,15,15h30c8.284,0,15-6.716,15-15S445.284,391,437,391z"/><path d="M105,181H75c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S113.284,181,105,181z"/><path d="M196,181h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S204.284,181,196,181z"/><path d="M287,181h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S295.284,181,287,181z"/><path d="M377,181h-30c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S385.284,181,377,181z"/><path d="M105,241H75c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S113.284,241,105,241z"/><path d="M196,241h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S204.284,241,196,241z"/><path d="M287,241h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S295.284,241,287,241z"/><path d="M377,241h-30c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S385.284,241,377,241z"/><path d="M105,301H75c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S113.284,301,105,301z"/><path d="M196,301h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S204.284,301,196,301z"/><path d="M287,301h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S295.284,301,287,301z"/><path d="M105,361H75c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S113.284,361,105,361z"/><path d="M196,361h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S204.284,361,196,361z"/><path d="M287,361h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S295.284,361,287,361z"/></svg></button>'
        + '</span>';
    }
    ...
    if(txtEl){
      ...
      if(checked){
        ...
        wrap.innerHTML = '<span class="proto-ci-date" id="proto-ci-datelbl-'+protoId+'-'+phaseId+'-'+idx+'">'+dateLbl+'</span>'
          + '<button class="proto-ci-cal-btn" onclick="event.stopPropagation();_protoEditCheckedAt(\''+escJS(String(protoId))+'\',\''+escJS(phaseId)+'\','+idx+')" title="Modifier la date"><svg width="14" height="14" fill="currentColor" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M452,311.145V75c0-24.813-20.187-45-45-45h-15V15c0-8.284-6.716-15-15-15s-15,6.716-15,15v15h-60V15c0-8.284-6.716-15-15-15s-15,6.716-15,15v15h-92V15c0-8.284-6.716-15-15-15s-15,6.716-15,15v15H90V15c0-8.284-6.716-15-15-15S60,6.716,60,15v15H45C20.187,30,0,50.187,0,75v331c0,24.813,20.187,45,45,45h266.946c16.795,36.01,53.085,61,95.054,61c57.897,0,105-47.551,105-106C512,364.198,487.443,328.027,452,311.145z M30,75c0-8.271,6.729-15,15-15h15v15c0,8.284,6.716,15,15,15s15-6.716,15-15V60h60v15c0,8.284,6.716,15,15,15s15-6.716,15-15V60h92v15c0,8.284,6.716,15,15,15s15-6.716,15-15V60h60v15c0,8.284,6.716,15,15,15s15-6.716,15-15V60h15c8.271,0,15,6.729,15,15v46H30V75z M303.056,421H45c-8.271,0-15-6.729-15-15V151h392v151.076c-4.901-0.704-9.907-1.076-15-1.076c-57.897,0-105,47.103-105,105C302,411.092,302.365,416.098,303.056,421z M407,482c-41.355,0-75-34.093-75-76c0-41.355,33.645-75,75-75c41.355,0,75,33.645,75,75C482,447.907,448.355,482,407,482z"/><path d="M437,391h-15v-15c0-8.284-6.716-15-15-15s-15,6.716-15,15v30c0,8.284,6.716,15,15,15h30c8.284,0,15-6.716,15-15S445.284,391,437,391z"/><path d="M105,181H75c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S113.284,181,105,181z"/><path d="M196,181h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S204.284,181,196,181z"/><path d="M287,181h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S295.284,181,287,181z"/><path d="M377,181h-30c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S385.284,181,377,181z"/><path d="M105,241H75c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S113.284,241,105,241z"/><path d="M196,241h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S204.284,241,196,241z"/><path d="M287,241h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S295.284,241,287,241z"/><path d="M377,241h-30c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S385.284,241,377,241z"/><path d="M105,301H75c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S113.284,301,105,301z"/><path d="M196,301h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S204.284,301,196,301z"/><path d="M287,301h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S295.284,301,287,301z"/><path d="M105,361H75c-8.284,0-15,6.716-15,15s6.716,15,15,15h30c8.284,0,15-6.716,15-15S113.284,361,105,361z"/><path d="M196,361h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S204.284,361,196,361z"/><path d="M287,361h-31c-8.284,0-15,6.716-15,15s6.716,15,15,15h31c8.284,0,15-6.716,15-15S295.284,361,287,361z"/></svg></button>';
  ...
  var items = history.map(function(ev, idx){
    ...
    var adminHtml = isAdmin
      ? '<div class="proto-tl-admin">'
        + '<button class="proto-tl-admin-btn" onclick="event.stopPropagation();_protoTlEditNote(\''+pid+'\','+idx+')" title="Modifier la note"><svg width="14" height="14" fill="currentColor" viewBox="0 0 348.882 348.882" xmlns="http://www.w3.org/2000/svg"><g><path d="M333.988,11.758l-0.42-0.383C325.538,4.04,315.129,0,304.258,0c-12.187,0-23.888,5.159-32.104,14.153L116.803,184.231c-1.416,1.55-2.49,3.379-3.154,5.37l-18.267,54.762c-2.112,6.331-1.052,13.333,2.835,18.729c3.918,5.438,10.23,8.685,16.886,8.685c0,0,0.001,0,0.001,0c2.879,0,5.693-0.592,8.362-1.76l52.89-23.138c1.923-0.841,3.648-2.076,5.063-3.626L336.771,73.176C352.937,55.479,351.69,27.929,333.988,11.758z M130.381,234.247l10.719-32.134l0.904-0.99l20.316,18.556l-0.904,0.99L130.381,234.247z M314.621,52.943L182.553,197.53l-20.316-18.556L294.305,34.386c2.583-2.828,6.118-4.386,9.954-4.386c3.365,0,6.588,1.252,9.082,3.53l0.419,0.383C319.244,38.922,319.63,47.459,314.621,52.943z"/><path d="M303.85,138.388c-8.284,0-15,6.716-15,15v127.347c0,21.034-17.113,38.147-38.147,38.147H68.904c-21.035,0-38.147-17.113-38.147-38.147V100.413c0-21.034,17.113-38.147,38.147-38.147h131.587c8.284,0,15-6.716,15-15s-6.716-15-15-15H68.904c-37.577,0-68.147,30.571-68.147,68.147v180.321c0,37.576,30.571,68.147,68.147,68.147h181.798c37.576,0,68.147-30.571,68.147-68.147V153.388C318.85,145.104,312.134,138.388,303.85,138.388z"/></g></svg></button>'
        + (idx > 0 ? '<button class="proto-tl-admin-btn del" onclick="event.stopPropagation();_protoTlDeleteEvent(\''+pid+'\','+idx+')" title="Supprimer cet événement"><svg width="14" height="14" fill="currentColor" viewBox="-40 0 427 427.00131" xmlns="http://www.w3.org/2000/svg"><path d="m232.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m114.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m28.398438 127.121094v246.378906c0 14.5625 5.339843 28.238281 14.667968 38.050781 9.285156 9.839844 22.207032 15.425781 35.730469 15.449219h189.203125c13.527344-.023438 26.449219-5.609375 35.730469-15.449219 9.328125-9.8125 14.667969-23.488281 14.667969-38.050781v-246.378906c18.542968-4.921875 30.558593-22.835938 28.078124-41.863282-2.484374-19.023437-18.691406-33.253906-37.878906-33.257812h-51.199218v-12.5c.058593-10.511719-4.097657-20.605469-11.539063-28.03125-7.441406-7.421875-17.550781-11.5546875-28.0625-11.46875h-88.796875c-10.511719-.0859375-20.621094 4.046875-28.0625 11.46875-7.441406 7.425781-11.597656 17.519531-11.539062 28.03125v12.5h-51.199219c-19.1875.003906-35.394531 14.234375-37.878907 33.257812-2.480468 19.027344 9.535157 36.941407 28.078126 41.863282zm239.601562 279.878906h-189.203125c-17.097656 0-30.398437-14.6875-30.398437-33.5v-245.5h250v245.5c0 18.8125-13.300782 33.5-30.398438 33.5zm-158.601562-367.5c-.066407-5.207031 1.980468-10.21875 5.675781-13.894531 3.691406-3.675781 8.714843-5.695313 13.925781-5.605469h88.796875c5.210937-.089844 10.234375 1.929688 13.925781 5.605469 3.695313 3.671875 5.742188 8.6875 5.675782 13.894531v12.5h-128zm-71.199219 32.5h270.398437c9.941406 0 18 8.058594 18 18s-8.058594 18-18 18h-270.398437c-9.941407 0-18-8.058594-18-18s8.058593-18 18-18zm0 0"/><path d="m173.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/></svg></button>' : '')
        + '</div>'
      : '';
...
/* Le paramètre COUPABLE est celui qui a blessé : on le réduit au départ, on le
   reconstruit à la fin — c'est lui, le traitement. Les paramètres INNOCENTS
   partent à la tolérance du coureur et n'y touchent plus : les développer
   reviendrait à charger un tissu qui n'a rien demandé, et geler le coupable
   sans jamais le remonter ne ramène personne à la course.

   `gele` et `moteur` désignaient deux paramètres différents — on gelait le
   coupable et on développait l'innocent. C'était l'inverse de la logique de la
...
var CAP_LIBELLES = {
  ...
  intensite:'allure (minutes de qualité)', amplitude:'amplitude de foulée',
...
/* Répartition des rôles sur la semaine. La séance de qualité n'existe que si
   la semaine porte des minutes d'intensité — c'est la trajectoire d'intensité
   qui décide, pas l'axe : sur Charge elle vaut zéro pendant la décharge puis
   monte, sur Répétition elle est là dès la première semaine. */
function _capRolesSemaine(axe, frequence, avecQualite) {
  var ax = CAP_AXES[axe] || CAP_AXES.charge;
  var roles = [];
  if (ax.technique) roles.push('technique');
  if (avecQualite) roles.push('qualite');
  // Une sortie longue n'a de sens que si le volume d'un seul tenant n'est pas
  // le paramètre coupable, et qu'il reste de la place dans la semaine.
  if (!ax.sansLongue && roles.length < frequence - 1) roles.push('longue');
...
/* Séance de qualité construite À PARTIR des minutes d'intensité voulues, et
   non d'une part du volume hebdomadaire. C'est l'inversion qui rend l'allure
   pilotable : `intensiteMin` est la grandeur que la trajectoire fait monter,
   la séance n'est que sa mise en forme.
...
function _capBuildSemaineN(p, opts, nbSorties, frequence) {
  ...
  // La qualité n'est plus une part du volume : c'est la trajectoire
  // d'intensité qui la commande. Sans minutes de Z3+, pas de séance de
  // qualité — l'axe Charge en décharge n'en produit aucune, par construction.
  var polarise = !fractionne && _capPolarisable(p, volumeMin, nbSorties);
  var avecQualite = polarise && intensite > 0;
  var roles = polarise ? _capRolesSemaine(p.axe, nbSorties, avecQualite)
...
/* Minutes de Z3+ effectivement produites par une semaine. Sert au contrôle
   plutôt qu'à l'affichage : c'est la grandeur que la trajectoire prétend
   piloter, autant pouvoir la mesurer sur le résultat. */
function _capIntensiteSemaine(seances) {
  return (seances || []).reduce(function(t, s) {
    return t + (s.segments || []).reduce(function(u, g) {
      return u + (CAP_ZONE_COEF[g.zone] >= CAP_ZONE_COEF.Z3 ? (g.reps || 1) * (g.minutes || 0) : 0);
    }, 0);
  }, 0);
}
...
/* Volume que le coureur tolère AUJOURD'HUI, en minutes par semaine. Quand le
   volume n'est pas le paramètre coupable, c'est là que le programme démarre —
   et il n'en bouge plus. La plus longue sortie tolérée est l'expression la
   plus directe de cette tolérance : elle ne peut pas rester un simple plafond. */
function _capVolumeTolere(p, frequence) {
  var cible = _capVersMin(Math.max(1, p.cibleHebdo), p);
  var declaree = 0;
  // La charge chronique prime : c'est ce que le coureur encaisse RÉELLEMENT.
  // La plus longue sortie tolérée ne prend le relais que faute de mieux.
  if (p.chronique > 0)             declaree = _capVersMin(p.chronique, p);
  else if (p.plusLongueSortie > 0) declaree = _capVersMin(p.plusLongueSortie, p) * frequence;
  // Rien de déclaré — ni charge chronique, ni sortie tolérée : il n'y a aucune
  // tolérance à préserver. C'est une reprise depuis zéro, et la cible devient
  // le seul repère. Figer le volume à la sortie de l'échelle course/marche
  // laisserait un post-opéré à 16 min de course pour toujours.
  if (!declaree) return cible;
  // La tolérance peut dépasser ce que le praticien veut prescrire : quatre
  // sorties de 12 km font 48 km/sem même s'il en visait 45. La cible saisie
  // reste un plafond — c'est sa décision, pas une déduction.
  return Math.min(declaree, cible);
}

/* Minutes hebdomadaires en Z3+. À défaut de saisie, une part du volume — 15 %,
   l'ordre de grandeur d'un coureur qui a retrouvé sa qualité sans être en
   préparation spécifique. Le volume de référence dépend du rôle : la CIBLE
   quand l'intensité est ce qu'on reconstruit, le volume de DÉPART quand elle
   est seulement maintenue — un paramètre innocent ne progresse pas, il ne peut
   donc pas être calé sur une cible qu'il n'atteindra jamais. */
function _capIntensitePour(p, volumeReferenceMin) {
  if (p.cibleIntensite > 0) return p.cibleIntensite;
  return Math.max(CAP_SEUILS.intensitePlancher, Math.round(volumeReferenceMin * 0.15));
}

/* Zone de la séance de qualité, graduée sur l'avancement de la remontée. Les
   zones disponibles viennent de l'axe : l'axe Amplitude plafonne à Z3, parce
   que corriger une foulée à pleine vitesse n'a pas de sens. C'est la seule
   lecture de `zonesDegel` — sans elle, la table décrivait une règle que le
   moteur n'appliquait pas. */
function _capZoneQualite(ax, intensiteSemaine, cibleIntensite) {
  var zones = (ax.zonesDegel || ['Z3', 'Z4']).filter(function(z) {
    return CAP_ZONE_COEF[z] >= CAP_ZONE_COEF.Z3;
  });
  if (!zones.length) return 'Z3';
  var part = cibleIntensite > 0 ? intensiteSemaine / cibleIntensite : 0;
  var i = Math.floor(part * zones.length);
  return zones[Math.max(0, Math.min(zones.length - 1, i))];
}

/* Semaines de décharge avant de rouvrir le paramètre coupable. L'os signale
   tard : on lui en laisse une de plus. Elles ne s'ajoutent que s'il n'y a pas
   déjà une phase de construction du volume — cette phase-là ne comporte aucune
   séance de qualité, elle EST une décharge pour l'allure, et la faire suivre
   d'une décharge supplémentaire ne ferait qu'immobiliser le plan. */
function _capSemDecharge(p) {
...
function _capRender() {
  ...
  Object.keys(byWeek).sort(function(a, b) { return +a - +b; }).forEach(function(w) {
    ...
    var wkBadge = st.load != null
      ...
      : '';
    // Minutes de qualité de la semaine : sur un axe Charge c'est LA grandeur
    // qui progresse, et sans elle l'écran ne montre qu'un volume immobile.
    var z3 = Math.round(_capIntensiteSemaine(weekSessions));
    var z3Badge = z3 > 0
      ? '<span style="font-size:.68rem;color:#B45309;font-weight:600;margin-left:8px;">' + z3 + '′ Z3+</span>'
      : '';
    ...
    html += '<div class="cap-week-hdr">Semaine ' + w + deloadTag + wkBadge + z3Badge + '</div>';
...
function _capGetTrend(seanceId) {
  ...
  var hits = _cloudCalEvents.filter(function(e){
    ...
    return String(e.id) !== String(seanceId)
      && e.date <= curDate
      && nom.indexOf('CAP') === 0
      && _fbDouleur(e.athlete_feedback) !== null;
  });
  hits.sort(function(a,b){ return a.date < b.date ? 1 : -1; });
  return hits.slice(0,3).reverse().map(function(e){ return _fbDouleur(e.athlete_feedback); });
}
...
function _renderCalSelBar() {
  ...
  bar.innerHTML = '<span style="font-size:.85rem;font-weight:600;color:#fff;flex:1;">' + label + '</span>'
    + (n > 0
      ? '<button onclick="_calSelDeleteAll()" style="background:#ef4444;color:#fff;border:none;border-radius:8px;padding:8px 20px;font-size:.85rem;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap;"><svg style="vertical-align:middle;margin-right:4px" width="16" height="16" fill="currentColor" viewBox="-40 0 427 427.00131" xmlns="http://www.w3.org/2000/svg"><path d="m232.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m114.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/><path d="m28.398438 127.121094v246.378906c0 14.5625 5.339843 28.238281 14.667968 38.050781 9.285156 9.839844 22.207032 15.425781 35.730469 15.449219h189.203125c13.527344-.023438 26.449219-5.609375 35.730469-15.449219 9.328125-9.8125 14.667969-23.488281 14.667969-38.050781v-246.378906c18.542968-4.921875 30.558593-22.835938 28.078124-41.863282-2.484374-19.023437-18.691406-33.253906-37.878906-33.257812h-51.199218v-12.5c.058593-10.511719-4.097657-20.605469-11.539063-28.03125-7.441406-7.421875-17.550781-11.5546875-28.0625-11.46875h-88.796875c-10.511719-.0859375-20.621094 4.046875-28.0625 11.46875-7.441406 7.425781-11.597656 17.519531-11.539062 28.03125v12.5h-51.199219c-19.1875.003906-35.394531 14.234375-37.878907 33.257812-2.480468 19.027344 9.535157 36.941407 28.078126 41.863282zm239.601562 279.878906h-189.203125c-17.097656 0-30.398437-14.6875-30.398437-33.5v-245.5h250v245.5c0 18.8125-13.300782 33.5-30.398438 33.5zm-158.601562-367.5c-.066407-5.207031 1.980468-10.21875 5.675781-13.894531 3.691406-3.675781 8.714843-5.695313 13.925781-5.605469h88.796875c5.210937-.089844 10.234375 1.929688 13.925781 5.605469 3.695313 3.671875 5.742188 8.6875 5.675782 13.894531v12.5h-128zm-71.199219 32.5h270.398437c9.941406 0 18 8.058594 18 18s-8.058594 18-18 18h-270.398437c-9.941407 0-18-8.058594-18-18s8.058593-18 18-18zm0 0"/><path d="m173.398438 154.703125c-5.523438 0-10 4.476563-10 10v189c0 5.519531 4.476562 10 10 10 5.523437 0 10-4.480469 10-10v-189c0-5.523437-4.476563-10-10-10zm0 0"/></svg>Supprimer</button>'
      : '')
