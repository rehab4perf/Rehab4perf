...
{ id:'programme', emoji:'📋', titre:'Programme & agenda', articles:[
  ...
  { id:'templates', titre:'Réutiliser une séance : les templates',
    intro:'Un template est un morceau de séance mis de côté pour le réutiliser : une séance entière, une étape, ou un morceau de bloc. Le geste est toujours le même — il s\'ajoute à ce que vous avez déjà.',
    etapes:[
      ...
      { t:'Étiquetez la pathologie et le sport', x:'Deux champs libres à l\'enregistrement — « LCA », « course à pied ». Ce sont eux qui permettront de retrouver la bonne séance : un nom de template se lit, il ne se filtre pas. Les étiquettes déjà utilisées vous sont proposées, pour éviter que « LCA » et « L.C.A. » coexistent sans le vouloir.' },
      { t:'Une catégorie étiquette tous ses templates', x:'Renseignez la pathologie sur la catégorie plutôt que sur chacun de ses templates : ils en héritent. Un template peut porter sa propre étiquette si elle diffère — la sienne l\'emporte alors.' },
      { t:'Créer un template de zéro', x:'Le bouton « Nouveau template », en haut du panneau Séances & templates, ouvre le builder vide, sans patient ni date : vous composez, puis « Enregistrer le template ». Si une séance était en cours dans le builder, elle est mise de côté et vous la retrouvez intacte en sortant — rien n\'est effacé.' },
      { t:'Seuls le nom et les étiquettes sont demandés', x:'La catégorie, son numéro d\'ordre et l\'emoji sont regroupés sous « Classement », replié : ils servent à ranger, aucun n\'est obligatoire.' },
      { t:'Ranger un template dans une catégorie', x:'« Nouveau template ici » sous une catégorie ouvre le même builder vide, catégorie et numéro d\'ordre déjà remplis. Une catégorie n\'est qu\'un regroupement de templates — à ne pas confondre avec les Protocoles de rééducation, qui s\'assignent à un patient et créent des rappels.' },
      { t:'Enregistrer la séance en cours comme template', x:'L\'autre chemin, quand une séance composée pour un patient mérite d\'être gardée : le bouton « Sauvegarder » propose de l\'envoyer dans les templates plutôt que chez le patient.' },
      { t:'Faire un template d\'une séance déjà enregistrée', x:'Le bouton « Faire un template » sur chaque séance sauvegardée évite de la recharger dans le builder pour l\'enregistrer une seconde fois. C\'est souvent après coup, une fois la séance composée pour un patient, qu\'on se dit qu\'on la refera. La séance en cours dans le builder n\'est pas touchée.' }
    ]},
