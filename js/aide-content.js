...
{ id:'programme', emoji:'📋', titre:'Programme & agenda', articles:[
  ...
  { id:'etapes', titre:'Regrouper des blocs en étapes',
    ...
    etapes:[
      ...
      { t:'Enregistrez une étape comme répertoire', x:'Le bouton dédié sauvegarde uniquement les blocs de cette étape, pas toute la séance — pratique pour réutiliser un échauffement type. Comme tout répertoire, il s\'ajoutera ensuite à la séance en cours, en copie indépendante et sans lien avec l\'original.' },
  ...
  { id:'templates', titre:'Les répertoires d\'exercices',
    intro:'Un répertoire n\'est pas une séance toute prête : c\'est une sélection d\'exercices retenus pour un profil — une bibliothèque affinée, extraite de la grande. On ne l\'envoie pas au patient, on y pioche pour composer sa séance. Un répertoire peut tenir en un bloc, une étape, ou une séance entière.',
    etapes:[
      { t:'Consulter un répertoire depuis l\'agenda', x:'Cliquez sur sa carte : elle se déplie sur place et montre ses blocs et exercices. Le clic ne redirige plus vers le builder — vous regardez avant de choisir.' },
      { t:'« Ouvrir dans le builder »', x:'Dans la carte dépliée, ce bouton ouvre une séance NEUVE avec le répertoire dedans — exactement comme « + Séance », mais déjà rempli. Ce que vous aviez dans le builder n\'est pas concerné : le répertoire ne s\'ajoute jamais à une séance déjà planifiée.' },
      { t:'Planifier directement, sans passer par le builder', x:'Le bouton « + » sur une carte crée une copie du répertoire pour le patient sélectionné, puis ouvre le même écran que « Planifier » depuis le builder : « Cocher les jours sur l\'agenda » en premier, la saisie date par date en repli. C\'est le même geste des deux côtés, avec ou sans passage par le builder.' },
      { t:'Un répertoire s\'ajoute, il ne remplace jamais', x:'Son contenu vient s\'ajouter à la suite de la séance en cours. Rien n\'est écrasé, donc rien ne se perd sans que vous l\'ayez demandé. Si vous voulez repartir du répertoire seul, videz la séance d\'abord : « Vider » est là pour ça.' },
      { t:'Composer une séance à partir de plusieurs répertoires', x:'C\'est le geste central : dans le builder, l\'onglet Répertoires permet de piocher un échauffement type, puis un bloc de renforcement, puis un finisher — chacun venant d\'un répertoire différent. Chaque ajout est une copie indépendante, modifier l\'une ne touche pas l\'original.' },
      { t:'Les étapes suivent leur contenu', x:'Si le répertoire contient des étapes, elles arrivent avec lui, sous leur nom et leur couleur. Elles ne se mélangent pas avec celles déjà en place. Un répertoire sans étape s\'ajoute hors étape, même si votre séance se terminait par une.' },
      { t:'Piocher un seul bloc plutôt que tout le répertoire', x:'Dans l\'onglet Répertoires du builder, dépliez un répertoire pour voir ses blocs et ajoutez celui qui vous intéresse. Vous pouvez aussi n\'ajouter qu\'un exercice. Le reste du répertoire n\'est pas touché.' },
      ...
      { t:'« Ajouter tous les blocs » garde le découpage', x:'C\'est le seul chemin qui importe les étapes du répertoire : là, le découpage fait sens puisque vous reprenez la séance entière.' },
      { t:'Créer un répertoire de zéro', x:'Le bouton « Nouveau répertoire », en haut du panneau Séances & répertoires, ouvre le builder vide, sans patient ni date : vous composez, puis « Enregistrer le répertoire ». Si une séance était en cours dans le builder, elle est mise de côté et vous la retrouvez intacte en sortant — rien n\'est effacé.' },
      { t:'« Mettre à jour le répertoire »', x:'Ce bouton n\'apparaît que si le répertoire est seul dans la séance en cours de composition. Dès que vous composez à partir de plusieurs sources, la séance n\'est plus ce répertoire : le bouton propose alors d\'en enregistrer un nouveau, pour ne pas écraser l\'original avec autre chose.' },
      { t:'Étiquetez la pathologie et le sport', x:'Deux champs libres à l\'enregistrement — « LCA », « course à pied ». Ce sont eux qui permettront de retrouver le bon répertoire : un nom se lit, il ne se filtre pas. Les étiquettes déjà utilisées vous sont proposées, pour éviter que « LCA » et « L.C.A. » coexistent sans le vouloir.' },
      ...
      { t:'Ajouter une phase à un protocole', x:'« Nouvelle phase » sous un protocole ouvre le même builder vide, avec le protocole et le numéro de phase déjà remplis. Une phase n\'est rien d\'autre qu\'un répertoire rangé dans un protocole.' }
    ]},
...
{ id:'athlete', emoji:'📱', titre:'Espace athlète', articles:[
  ...
  { id:'push-athlete', titre:'Notifications push sur le téléphone de l\'athlète 🔔',
    ...
    etapes:[
      { t:'Ce qui déclenche une notification', x:'Planifier une nouvelle séance (quel que soit le mode : ajout rapide, répertoire, builder, protocole/cycle), déplacer ou dupliquer une séance existante, et envoyer un message patient.' },
