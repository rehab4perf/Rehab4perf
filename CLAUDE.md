...
## Où trouver le reste — skills de domaine

Ce fichier ne garde que ce qui vaut pour **tout** le dépôt. Le savoir propre à
un domaine vit dans `.claude/skills/`, et se charge quand on y touche — il
pesait 81 k caractères chargés à chaque session, y compris pour une question
d'une ligne.

| Domaine | Skill | Ce qu'elle porte |
|---|---|---|
| Compte-rendu médecin | `cr-medecin` | Générateur de CR, lettre et PDF, tableaux de mesures, côté nommé, marquage « réévalué », lexique des tests |
| Bilan clinique | `bilan-clinique` | Mobilités actif/passif, tests de force et symétrie, asymétrie affichée, bilans de suivi et antérieurs, date du bilan, motifs |
| Graphiques d'Évolution | `graphiques-evolution` | Les trois constructeurs SVG, en-tête chiffré, placement des étiquettes, récolte par le CR |
| Builder et programmes | `builder-programme` | Blocs et étapes, templates et protocoles, empreinte de séance, cycles de l'athlète, générateur CAP |

Rien n'a été supprimé : ces sections sont déplacées telles quelles. **Avant de
modifier un de ces domaines, charger la skill correspondante** — elle contient
les pièges déjà refermés, et les rouvrir coûte cher.

## Skills de méthode

Trois rituels sont écrits une fois pour toutes, à côté des skills de domaine :
...
Ce fichier garde la mémoire de ce qui vaut **partout** ; les skills de domaine
